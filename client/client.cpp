...
void rpc_create_slice(struct ib_resources_t *ib_resources){
	rpc_call(ib_resources,
			CREATE_SLICE,
			NULL,
			0,
...
void	rpc_destory_slice(struct ib_resources_t *ib_resources){
	rpc_call(ib_resources,
			DESTROY_SLICE,
			NULL,
			0,
...
void	rpc_slice_memory_allocate(struct ib_resources_t *ib_resources){
	rpc_call(ib_resources,
			SLICE_MEMORY_ALLOCATE,
			NULL,
			0,
			NULL,
			NULL);
...
void	rpc_slice_register_function(struct ib_resources_t *ib_resources){
	rpc_call(ib_resources,
			SLICE_REGISTER_FUNCTION,
			NULL,
			0,
			NULL,
			NULL);
...
void	rpc_slice_call_function(struct ib_resources_t *ib_resources){
	rpc_call(ib_resources,
			SLICE_CALL_FUNCTION,
			NULL,
			0,
...
int main(int argc, char *argv[]) {
	...
	rpc_create_slice(ib_resources);
	

	sleep(20);
	printf("slice memory allocate.\n");
	rpc_slice_memory_allocate(ib_resources);


	sleep(20);
	printf("slice register function.\n");
	rpc_slice_register_function(ib_resources);

	sleep(20);
	printf("slice call function.\n");
	rpc_slice_call_function(ib_resources);

	sleep(20);
	printf("destroy slice.\n");
	rpc_destory_slice(ib_resources);
